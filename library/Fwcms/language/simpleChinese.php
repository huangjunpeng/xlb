...
return array(
    ...
    "'%value%' has not only alphabetic and digit characters" => "不是字母或数字",
    "'%value%' is an empty string"                           => "输入值不可以为空",
    "'%value%' has not only alphabetic characters"           => "包含非英文字母的字符",
    "isValid"                                                => "错误",
    "'%value%' must contain between 13 and 19 digits"        => "必须是一个13到19位之间的数字",
    "Luhn algorithm (mod-10 checksum) failed on '%value%'"   => "Luhn algorithm (mod-10 checksum) failed on '%value%'",
    "'%value%' is not between '%min%' and '%max%', inclusively" => "不在 '%min%' 和 '%max%'之间",
    "'%value%' is not strictly between '%min%' and '%max%'"     => "必须大于 '%min%' ，小于 '%max%'",
    "'%value%' is not of the format YYYY-MM-DD"                 => "必须是类似 YYYY-MM-DD 这样的日期格式",
    "'%value%' does not appear to be a valid date"              => "是一个错误的日期",
    "'%value%' does not fit given date format"                  => "日期格式错误",
    "'%value%' contains not only digit characters"              => "不是数字",
    ...
    "'%value%' exceeds the allowed length"                                                   => "不是一个电子邮件地址",
    "'%value%' does not appear to be a float"                                                => "不是一个浮点数",
    ...
    "'%value%' was not found in the haystack"=>"不在可选范围之内",
    "'%value%' does not appear to be an integer"=>"不是一个整数",
    "'%value%' does not appear to be a valid IP address"=>"不是一个合法的IP地址",
    "'%value%' is not less than '%max%'"=>"输入值必须小于 '%max%'",
    "'%value%' does not match against pattern '%pattern%'"=>"不符合输入规则",
    "'%value%' is less than %min% characters long"=>"字符长度小于 %min% 位",
    "'%value%' is greater than %max% characters long"=>"字符长度超过 %max% 位",
	"The file '%value%' has a false extension"=>"不能上传此文件"
);
?>
<?php
/*return array(
