...
class UserOrder extends Common
{
	...
	private static function _GetBaseQuery($db , $options)
	{
		...
		$select = $db->select();
		$select->from(array('ua' => $prefix."user_order"), array());	
				
		
		if(strlen($options['userId']) > 0){
		    $select->where("ua.userId = ?",$options['userId']);
		}				
		if(strlen($options['orderNo']) > 0){
		    $select->where("ua.orderNo = ?",$options['orderNo']);
		}
                if(strlen($options['status']) > 0){
		    $select->where("ua.status < ?",$options['status']);
		}
		return $select;
	}
	 ...
	 * @return bool
	 */
	function delDataByUser($userId,$attUid)
	{
	    return $this->delete("userId = ".$userId." and attUid = ".$attUid);
	}	
