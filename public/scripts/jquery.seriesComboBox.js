	...
	yijs.Ld.prototype = {
		...
		load : function(selectIndex,parent_id,callback){
			...
			if(_that.cache[parent_id]){
				...
				var _parentIdField = this.options.field['url_field'];
				...
				_ajaxOptions.success = function(data){
						//遍历数据，获取html字符串
						var _h = _that._getOptionsHtml(data,selectIndex);
		 ...
		 */
		_getOptionsHtml : function(data,selectIndex){
			var _that = this;
			var ajaxOptions = this.options.ajaxOptions;
			var dataType = ajaxOptions.dataType;
			var field = this.options.field; 
			var _h = "";
			_h = _getOptions(data,dataType,field,selectIndex).join("");;
			/**
			 * 获取选择框项html代码数组
			 ...
			 * @return {Array} aStr 
			 */
			function _getOptions(data,dataType,field,selectIndex){
				
				var optionClass = _that.OPTIONS_CLASS;
				var selectnum = selectIndex+1;
				var aStr = [];
				var id,name;
				if (dataType == "json") {
					$.each(data,function(i){
					    id   = data[i][field.region_id];
						name = data[i][field.region_name];//+selectnum+"-"
